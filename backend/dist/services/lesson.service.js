"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.reorderLessons = exports.deleteLesson = exports.updateLesson = exports.createLesson = void 0;
const prisma_1 = require("../config/prisma");
const course_service_1 = require("./course.service");
const validateAssessmentJSON = (content) => {
    try {
        const data = JSON.parse(content);
        if (!data.title || typeof data.title !== "string")
            throw new Error("Assessment title is required.");
        if (!Array.isArray(data.questions) || data.questions.length === 0)
            throw new Error("Assessment must have at least one question.");
        if (typeof data.passingScore !== "number")
            throw new Error("Passing score must be a number.");
        data.questions.forEach((q, index) => {
            if (!q.question || typeof q.question !== "string")
                throw new Error(`Question ${index + 1} text is missing.`);
            if (!Array.isArray(q.options) || q.options.length !== 4)
                throw new Error(`Question ${index + 1} must have exactly 4 options.`);
            if (typeof q.correctAnswerIndex !== "number" ||
                q.correctAnswerIndex < 0 ||
                q.correctAnswerIndex > 3) {
                throw new Error(`Question ${index + 1} has an invalid correctAnswerIndex (must be 0-3).`);
            }
        });
        return true;
    }
    catch (err) {
        const error = err;
        throw new Error(`Invalid Assessment JSON: ${error.message}`);
    }
};
const getModuleCourseId = async (moduleId) => {
    const module = await prisma_1.prisma.module.findUnique({
        where: { id: moduleId },
        select: { courseId: true },
    });
    if (!module)
        throw new Error("Module not found");
    return module.courseId;
};
const getLessonCourseId = async (lessonId) => {
    const lesson = await prisma_1.prisma.lesson.findUnique({
        where: { id: lessonId },
        select: { module: { select: { courseId: true } } },
    });
    if (!lesson)
        throw new Error("Lesson not found");
    return lesson.module.courseId;
};
const createLesson = async (moduleId, data, userId, userRole) => {
    const courseId = await getModuleCourseId(moduleId);
    await (0, course_service_1.verifyCourseOwnership)(courseId, userId, userRole);
    if (!data.type) {
        throw new Error("Lesson type is required");
    }
    if (data.type === "VIDEO") {
        data.content = undefined;
    }
    else if (data.type === "READING") {
        data.videoUrl = undefined;
        data.duration = undefined;
    }
    else if (data.type === "ASSESSMENT") {
        data.videoUrl = undefined;
        if (data.content) {
            validateAssessmentJSON(data.content);
        }
    }
    return await prisma_1.prisma.lesson.create({
        data: {
            ...data,
            moduleId,
        },
    });
};
exports.createLesson = createLesson;
const updateLesson = async (id, data, userId, userRole) => {
    const courseId = await getLessonCourseId(id);
    await (0, course_service_1.verifyCourseOwnership)(courseId, userId, userRole);
    const existingLesson = await prisma_1.prisma.lesson.findUnique({ where: { id } });
    if (!existingLesson)
        throw new Error("Lesson not found");
    const updateData = { ...data };
    const type = data.type || existingLesson.type;
    if (type === "VIDEO") {
        if (data.type || existingLesson.type === "VIDEO") {
            updateData.content = null;
        }
    }
    else if (type === "READING") {
        updateData.videoUrl = null;
        updateData.duration = null;
    }
    else if (type === "ASSESSMENT") {
        updateData.videoUrl = null;
        updateData.duration = null;
        if (updateData.content) {
            validateAssessmentJSON(updateData.content);
        }
    }
    return await prisma_1.prisma.lesson.update({
        where: { id },
        data: updateData,
    });
};
exports.updateLesson = updateLesson;
const deleteLesson = async (id, userId, userRole) => {
    const courseId = await getLessonCourseId(id);
    await (0, course_service_1.verifyCourseOwnership)(courseId, userId, userRole);
    return await prisma_1.prisma.lesson.delete({
        where: { id },
    });
};
exports.deleteLesson = deleteLesson;
const reorderLessons = async (lessonOrders, userId, userRole) => {
    if (lessonOrders.length > 0) {
        const courseId = await getLessonCourseId(lessonOrders[0].id);
        await (0, course_service_1.verifyCourseOwnership)(courseId, userId, userRole);
    }
    const updates = lessonOrders.map((item) => prisma_1.prisma.lesson.update({
        where: { id: item.id },
        data: { order: item.order },
    }));
    return await prisma_1.prisma.$transaction(updates);
};
exports.reorderLessons = reorderLessons;
