...
const InstructorLayout: React.FC = () => {
  ...
  const navItems: NavItem[] = useMemo(
    () => [
      ...
      },
      {
        ...
        icon: (
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14m0-4v4m0-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h8a2 2 0 002-2" />
          </svg>
        ),
      },
      {
  ...
  const currentPageTitle = navItems.find((item) => item.path === location.pathname)?.name || "Instructor";

  const Sidebar: React.FC = () => (
    <div className="flex flex-col h-full bg-white border-r border-gray-200 shadow-sm">
      <div className="p-6 border-b border-gray-100">
        <Link to="/" className="block group w-max transition-opacity hover:opacity-80">
          <img src={IMAGES.LOGO} alt="Coursera" className="h-7 w-auto transition-transform duration-300 group-hover:scale-105" />
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <span className={`transition-colors ${isActive ? "text-blue-600" : "text-gray-400 group-hover:text-gray-900"}`}>
                {item.icon}
              </span>
              <span className={`text-sm font-medium ${isActive ? "text-blue-600" : "text-gray-600 group-hover:text-gray-900"}`}>
                {item.name}
              </span>
            </Link>
          );
          ...
          <div className="px-6 lg:px-8 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <div className="hidden md:flex items-center bg-gray-50 rounded-lg px-4 py-2.5 border border-gray-200 group focus-within:bg-white focus-within:border-blue-300 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
                <svg className="w-4 h-4 text-gray-400 group-focus-within:text-blue-600 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
