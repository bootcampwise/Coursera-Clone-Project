    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-gray-200">
        <div>
          ...
          <p className="text-sm text-gray-500">Upload and manage lesson videos for your courses.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <section className="lg:col-span-2 bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Attach videos to lessons and track processing.</p>
            </div>
            <div className="min-w-[220px]">
              <label className="block text-xs font-medium text-gray-600 mb-2">Course</label>
              <select
          ...
          <div className="rounded-xl border-2 border-dashed border-gray-200 bg-gradient-to-br from-blue-50 to-gray-50 p-10 text-center">
            ...
            <p className="text-sm font-semibold text-gray-900">Drag & drop to upload</p>
            <p className="text-xs text-gray-500 mt-2">MP4 recommended. Keep file sizes reasonable.</p>
        ...
        <section className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          ...
          <p className="text-sm text-gray-500">Keep a consistent learner experience.</p>
          <div className="mt-4 space-y-3">
            {[
              { title: "Video Quality", desc: "1080p preferred, clear audio." },
              { title: "Naming", desc: "Module/Lesson naming should match curriculum." },
              { title: "Length", desc: "Aim for 5–12 minute lessons." },
              { title: "Captions", desc: "Add captions for accessibility." },
            ].map((g) => (
              <div key={g.title} className="p-3 rounded-lg bg-gray-50 border border-gray-200">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200">
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Lesson</th>
              ...
              {rows.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="text-sm font-semibold text-gray-900">{r.lesson}</div>
                    <div className="text-xs text-gray-500 mt-0.5">ID: {r.id}</div>
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-700">{r.course}</td>
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600">{r.uploadedAt}</td>
